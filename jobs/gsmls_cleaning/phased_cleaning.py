...
from gsmls_core.gsmls.utility_func import create_postgres_connection
...
def stage_one():
    ...
    final_data.to_parquet("/workspace/data/stage_one/refined_data.parquet", index=False,
...
def stage_two():
    ...
    df = spark.read.parquet("/workspace/data/stage_one/refined_data.parquet")
...
def stage_three(obj: GSMLSCleaning, mls_list: list):
    ...
    refined_data = (refined_data.pipe(GSMLSCleaning.reorder_zscore_cols)
                    .pipe(GSMLSCleaning.remove_columns, phase='stage_three'))
    refined_data['property_valuation'] = refined_data['assess_to_lp_zscore'].apply(GSMLSCleaning.property_valuation)
    refined_data.insert(10, 'property_valuation', refined_data.pop('property_valuation'))
    refined_data.insert(11, 'assess_to_lp_zscore', refined_data.pop('assess_to_lp_zscore'))
    ...
    print(f' ==== FIRST 10 MLSNUMs: {mls_list[:11]}')
    # obj.save_cleaned_data(refined_data)
    refined_data.to_parquet("/workspace/data/stage_two/stage_three_data.parquet", index=False,
                            engine='pyarrow', coerce_timestamps='us', allow_truncated_timestamps=True)
...
def main():
    ...
    clean_obj, target_mls_list = stage_one()
    ...
    stage_three(clean_obj, target_mls_list)
